...
public class ProcessNIPSCompResults {
		...
		for(int i = 0; i < Global.probNamesRef.size(); i ++) {
			...
			for(int j = 0; j < Global.ratioRef.size(); j ++) {
				...
				for(int t = 0; t < Global.timesRef.size(); t ++) {
					...
					for(int a = 0; a < Global.algorithmsRef.size(); a ++) {
						new Records().fileAppend(resDir + outputDir + theProb + "_AGG_" + theTime + "_" + Global.algorithmsRef.get(a), 
								theRatio + " " + avg.get(a) + " " + std.get(a), false);
					}
					//record individual file
					for(int a = 0; a < Global.algorithmsRef.size(); a ++) {
						new Records().fileAppend(resDir + outputDir + theProb + "_0_" +  theTime + "_" + Global.algorithmsRef.get(a), 
								theRatio + " " + ind.get(a), false);
					}
				}
			}
		}
